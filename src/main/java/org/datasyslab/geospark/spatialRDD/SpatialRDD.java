...
public abstract class SpatialRDD implements Serializable {
	...
	private static final long serialVersionUID = 639350790705153090L;
	/** The Constant logger. */
	...
	public boolean spatialPartitioning(GridType gridType) throws Exception {
		...
		if (this.boundaryEnvelope == null) {
			throw new Exception(
					"[AbstractSpatialRDD][spatialPartitioning] SpatialRDD boundary is null. Please call boundary() first.");
		}
		if (this.totalNumberOfRecords == -1) {
			throw new Exception(
					"[AbstractSpatialRDD][spatialPartitioning] SpatialRDD volume is unkown. Please call count() first.");
		}
		...
		if (gridType == GridType.EQUALGRID) {
			...
			HilbertPartitioning hilbertPartitioning = new HilbertPartitioning(objectSampleList, this.boundaryEnvelope,
					numPartitions);
			grids = hilbertPartitioning.getGrids();
		} else if (gridType == GridType.RTREE) {
			RtreePartitioning rtreePartitioning = new RtreePartitioning(objectSampleList, this.boundaryEnvelope,
					numPartitions);
			grids = rtreePartitioning.getGrids();
		} else if (gridType == GridType.VORONOI) {
			VoronoiPartitioning voronoiPartitioning = new VoronoiPartitioning(objectSampleList, this.boundaryEnvelope,
					numPartitions);
		...
		JavaPairRDD<Integer, Object> spatialNumberingRDD = this.rawSpatialRDD
				.flatMapToPair(new PairFlatMapFunction<Object, Integer, Object>() {
					@Override
	...
	public void buildIndex(final IndexType indexType, boolean buildIndexOnSpatialPartitionedRDD) throws Exception {
		if (buildIndexOnSpatialPartitionedRDD == false) {
					...
					if (indexType == IndexType.RTREE) {
						...
						while (spatialObjects.hasNext()) {
							...
							if (spatialObject instanceof Envelope) {
								...
								throw new Exception(
										"[AbstractSpatialRDD][buildIndex] Unsupported spatial partitioning method.");
							}
						}
			...
			JavaPairRDD<Integer, Iterable<Object>> groupBySpatialPartitionedRDD = this.spatialPartitionedRDD
					.groupByKey();
			this.indexedRDD = groupBySpatialPartitionedRDD
					.flatMapValues(new Function<Iterable<Object>, Iterable<Object>>() {
						@Override
